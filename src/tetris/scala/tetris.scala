...
case class TetrisWorld(piece: ((Int, Int), S.Shape), pile: S.Shape) extends World() {
  ...
  def keyEvent(key: String): World = {
    ...
    def fallworld(world: TetrisWorld): TetrisWorld = {
      val ((x, y), shape) = world.piece
      val world2 = TetrisWorld(((x, y+1), shape), pile)
      if (collision(world2) == false) fallworld(world2)
      else world
    }
    ...
    val world = {
      ...
      else if (key == "DOWN") fallworld(TetrisWorld(piece, pile))
