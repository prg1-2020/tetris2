...
object ShapeLib {
  ...
  val blockSymbols = List('I', 'J', 'T', 'O', 'Z', 'L', 'S', 'X')
  val blockColors = {
    val n = blockSymbols.length
    for (i <- Range(0, n)) yield (HSB(117f * i / n, 0.3f, 1))
  }
  ...
  val List(shapeI, shapeJ, shapeT, shapeO, shapeZ, shapeL, shapeS, shapeX) = allShapes
