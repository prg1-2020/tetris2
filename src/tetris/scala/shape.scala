...
object ShapeLib {
  ...
  def change(shape:Shape):Shape = {
    assert(wellStructured(shape))
    val shapeJ1 = rotate(shapeJ)
    val shapeJ2 = rotate(rotate(shapeJ))
    val shapeJ3 = rotate(rotate(rotate(shapeJ)))
    val shapeL1 = rotate(shapeL)
    val shapeL2 = rotate(rotate(shapeL))
    val shapeL3 = rotate(rotate(rotate(shapeL)))
    val shapeT1 = rotate(shapeT)
    val shapeT2 = rotate(rotate(shapeT))
    val shapeT3 = rotate(rotate(rotate(shapeT)))
    val shapeI1 = rotate(shapeI)
    val shapeS1 = rotate(shapeS)
    val shapeZ1 = rotate(shapeZ)
    shape match{

      case `shapeO` => shapeO
      //不変
      case `shapeI` => shapeJ1
      case `shapeJ1` => shapeL2
      case `shapeL2` => shapeT
      case `shapeT` => shapeI
      //第一グループ
      case `shapeJ` => shapeL
      case `shapeL` => shapeJ3
      case `shapeJ3` => shapeT2
      case `shapeT2` => shapeJ
      //第二グループ
      case `shapeS1` => shapeZ1
      case `shapeZ1` => shapeT3
      case `shapeT3` => shapeL3
      case `shapeL3` => shapeS1
      //第三グループ
      ...
      case `shapeJ2` => shapeL1
      case `shapeL1` => shapeT1
      case `shapeT1` => shapeJ2
      //第五グループ
    }
  }
